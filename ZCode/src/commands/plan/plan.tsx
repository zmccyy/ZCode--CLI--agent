...
import { resolvePlanCommandBehavior } from './planBehavior.js';
...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext, args: string): Promise<React.ReactNode> {
  ...
  const action = await resolvePlanCommandBehavior({
    args,
    currentMode,
    getPlanContent: getPlan,
    getPlanPath: getPlanFilePath,
    editorName,
    openPlanInEditor: editFileInEditor,
    renderPlanDisplay: async ({
      planContent,
      planPath,
      editorName: displayEditorName
    }) => {
      const display = <PlanDisplay planContent={planContent} planPath={planPath} editorName={displayEditorName} />;
      return renderToString(display);
    }
  });
  if (action.type === 'enable') {
    ...
    onDone(action.result, action.options);
    return null;
  }
  onDone(action.result, action.options);
  return null;
