/**
 * Marketplace manager for ZCode plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a ZCode bug, shouldn't hit the error file.
    return {}
  }
}
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `ZCode version. Run 'zcode marketplace remove ${name}' and ` +
