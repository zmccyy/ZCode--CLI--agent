...
| W1 | T2.8 Phase 2 第一波回归（S01/S06/S02 完善） | 无 | 27 条测试全绿（S01 品牌+诊断、S02 会话恢复 7 条、S06 Plan 模式 6 条） |
| W2 | T2.7 品牌批量替换（第 2 批：`src/tools/`、`src/commands/`） | W1-T2.7 | 已清理 216 处残留（第 1 批 107 + 第 2 批 109） |
| W2 | 冻结基线快照：`git tag v0.1.0-baseline` | W1 全部 | 已完成 |

- **Gate-In**：当前 74/75 测试通过。
- **Gate-Out**：`v0.1.0-baseline` 标签存在；品牌残留约 314 处（从 530 降到 ~216，主要集中在 src/screens/、src/services/、src/plugins/ 等尚未处理的目录）；S01/S02/S06 三条回归全绿。
